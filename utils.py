...
def plot_line(xs, ys_population, save_dir):
  ...
  trace_upper = Scatter(x=xs, y=ys_upper.numpy(), line=Line(color='aqua'), name='+1 Std. Dev.', showlegend=False)
  trace_mean = Scatter(x=xs, y=ys_mean.numpy(), fill='tonexty', fillcolor=std_colour, line=Line(color=mean_colour), name='Mean')
  trace_lower = Scatter(x=xs, y=ys_lower.numpy(), fill='tonexty', fillcolor=std_colour, line=Line(color='aqua'), name='-1 Std. Dev.', showlegend=False)
